...
class ReportService:
    ...
    def generate_html_report_content(
        ...
        repo_name: str, 
        llm_json_output: str, 
        chart_image_url: Optional[str] = None # <-- PARÂMETRO FALTANTE
    ) -> Tuple[str, str]:
        """
        Gera o conteúdo HTML final, usando uma imagem estática se a URL for fornecida.
        """
        try:
        ...
        # --- Lógica do Gráfico Estático ---
        chart_html = ""
        if chart_image_url:
            # Se a URL existir, usa a tag <img>
            ...
            # Se não houver URL, esconde o container (para emails)
            # ou tenta renderizar com Chart.js (para downloads)
            chart_json_obj = report_data.get("chart_json") if 'report_data' in locals() else None
...
{"" if chart_image_url else "<script>...</script>"} 
</body></html>
"""
    ...
    def generate_report_content(
        ...
        format: str = "html",
        chart_image_url: Optional[str] = None # <-- O PARÂMETRO FALTANTE
    ) -> Tuple[str, str, str]:
        """
        Gera o conteúdo final (HTML ou JSON) para o relatório.
        """
        if format.lower() == "html":
            content_string, filename = self.generate_html_report_content(
                repo_name, 
                content_json_str, 
                chart_image_url # <-- Passa a URL para o gerador de HTML
            )
